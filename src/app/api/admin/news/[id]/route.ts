...
export const PATCH = withErrorHandler(async (req: NextRequest, context: RouteContext) => {
    ...
    if (data.slug !== undefined) {
        slug = data.slug || ensureSlug({ title: data.title || existingNews.title });
    ...
    const isPublishing = data.status === 'PUBLISHED' && existingNews.status !== 'PUBLISHED';
    const publishedAt = isPublishing ? new Date() : (data.publishedAt ? new Date(data.publishedAt) : existingNews.publishedAt);

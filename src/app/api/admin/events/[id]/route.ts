...
export const PATCH = withErrorHandler(async (req: NextRequest, context: RouteContext) => {
    ...
    if (data.slug !== undefined) {
        slug = data.slug || ensureSlug({ title: data.title || existingEvent.title });
    ...
    const isPublishing = data.status === 'PUBLISHED' && existingEvent.status !== 'PUBLISHED';
    const publishedAt = isPublishing ? new Date() : (data.publishedAt ? new Date(data.publishedAt) : existingEvent.publishedAt);

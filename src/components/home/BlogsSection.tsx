  ...
  return (
    <section id="blog" className="relative bg-[#0356c2] py-20">
      <div className="max-w-[1280px] mx-auto px-6">
        {/* Title */}
        <h2 className="text-white text-[60px] text-center mb-16">{t('title')}</h2>
          ...
          {blogs.map((blog) => {
            ...
            return (
              ...
              >
                ...
                {/* Content */}
                <div className={`flex-1 ${blog.alignment === 'right' ? 'text-right' : ''}`}>
                  <h3 className="text-white text-[30px] mb-3">{t(`items.${key}.title`)}</h3>
                  <div className="flex items-center gap-4 mb-4">
                    {blog.alignment === 'right' ? (
                      <>
                        <p className="text-[#9cd4fc] text-[15px] ml-auto">{formatDate(blog.date, locale)}</p>
                        <p className="text-[#ffe500] text-[15px]">{blog.author}</p>
                      </>
                    ) : (
                      <>
                        <p className="text-[#ffe500] text-[15px]">{blog.author}</p>
                  ...
                  <p className="text-[#ededed] text-[15px] leading-relaxed mb-4">
                    {t(`items.${key}.excerpt`)}
                  </p>
